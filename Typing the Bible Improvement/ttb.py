...
if len(df_results) > 0: # If df_results is empty, then there's
    ...
    df_results['Local_Start_Time'] = pd.to_datetime(df_results['Local_Start_Time'], format='mixed').astype('datetime64[us]')
    df_results['UTC_Start_Time'] = pd.to_datetime(
    df_results['UTC_Start_Time']
...
total_characters = df_Bible['Characters'].sum()
typed_characters = df_Bible.loc[df_Bible['Typed'] == 1, 'Characters'].sum()
completion_percentage = (typed_characters / total_characters) * 100
...
fig_tree_map_books_chapters_verses = px.treemap(
    ...
    title=f'Proportions of Bible Books and Chapters That Have Been Typed<br>Total Completion: {completion_percentage:.2f}%'
)

# Center the title
fig_tree_map_books_chapters_verses.update_layout(
    title={
        'text': f'Proportions of Bible Books and Chapters That Have Been Typed<br>Total Completion: {completion_percentage:.2f}%',
        'x': 0.5,
        'xanchor': 'center'
    }
)
...
if (len(df_results.query("Mistake_Free_Test == 0")) >= 1) & (
    ...
    inplace = True)
    df_wpm_by_mistake_free_status['Mistake_Free_Test'] = df_wpm_by_mistake_free_status['Mistake_Free_Test'].replace(
    {0: 'No', 1: 'Yes'})
...
aggfunc = {'WPM':'mean', 'Count':'sum'}).reset_index()
df_results_by_within_session_test_number = df_results_by_within_session_test_number.query("Count >= 10")
...
setting component, so charts that rely on it will not be created.")
    ...
    data_source_pivot = data_source_pivot.sort_values(y_value, ascending=False)
